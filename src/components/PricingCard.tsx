  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl p-10 transition-all duration-300 ${
        isPro 
          ? 'bg-gradient-to-br from-blue-600 to-purple-600 text-white shadow-2xl scale-105 border-4 border-white dark:border-slate-800' 
    ...
    >
      {isPro && badge && (
        <div className="absolute -top-5 left-1/2 -translate-x-1/2">
          <div className="flex items-center gap-2 px-6 py-2 bg-gradient-to-r from-yellow-400 to-orange-400 text-slate-900 rounded-full text-sm font-bold shadow-xl">
      ...
      <div className="mb-8">
        ...
        <div className={`text-5xl font-bold ${isPro ? 'text-white' : 'text-blue-600 dark:text-blue-400'}`}>{price}</div>
      </div>
        ...
        {features.map((feature, index) => (
          <li key={index} className="flex items-start gap-4">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
              ...
              <Check className={`w-5 h-5 ${isPro ? 'text-white' : 'text-blue-600 dark:text-blue-400'}`} />
            </div>
            <span className={`leading-relaxed text-lg ${isPro ? 'text-white/95' : 'text-slate-600 dark:text-slate-300'}`}>{feature}</span>
          </li>
        ))}
      </ul>
      ...
      <a
        href={ctaLink}
        className={`flex items-center justify-center gap-3 w-full py-5 rounded-2xl font-bold text-lg transition-all duration-300 ${
          isPro
